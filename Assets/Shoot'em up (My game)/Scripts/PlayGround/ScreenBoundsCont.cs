...
public class ScreenBoundsCont : MonoBehaviour
{
    ...
    [SerializeField] private float _screenIndent = 0;
    ...
    void Start()
    {
        ...
        foreach (BoxCollider2D Collider in _screenBoundsColliders)
        {
            if (Collider.size.x > Collider.size.y)
            {
                if (indexHor == 0)
                {
                    Collider.offset = new Vector2(0, bottomBoundary - _screenIndent);
                    Collider.size = new Vector2((leftBoundary - _screenIndent) * -2, 0.5f);
                    indexHor = 1;
                }
                else
                {
            ...
            else
            {
                if (indexVert == 0)
                {
                    Collider.offset = new Vector2(leftBoundary - _screenIndent, 0);
                    Collider.size = new Vector2(0.5f, (bottomBoundary - _screenIndent) * -2);
                    indexVert = 1;
                }
                else
                {
